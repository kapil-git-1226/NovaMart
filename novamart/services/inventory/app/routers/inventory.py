...
# Returns all products and their quantities for a given store
...
# Returns stock info for a single product in a store
...
# Returns products whose quantity is at or below the reorder level
...
# Adjust stock for a product in a store (IN / OUT / ADJ)
...
def adjust_stock(body: StockAdjust, db: Session = Depends(get_db)):
    ...
    if body.type == "IN":
        ...
        if inv.quantity < body.qty:
            raise HTTPException(
                status_code=400,
                detail=f"Insufficient stock. Available: {inv.quantity}, Requested: {body.qty}"
            )
        inv.quantity -= body.qty
    else:  # ADJ — set to exact value
        inv.quantity = body.qty
...
# View stock movement history for a store (last 100 entries)
